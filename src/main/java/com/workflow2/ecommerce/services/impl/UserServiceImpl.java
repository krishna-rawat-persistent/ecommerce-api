...
public class UserServiceImpl implements UserService {
    ...
    public Response login(Login login) {
        ...
        if(user!=null){
            ...
            if(passwordMatched){
                Optional<User> validUser = repo.findOneByUserNameAndPassword(login.getUserName(),encryptedPassword);
                if(validUser.isPresent()){
                    return Response.builder().status(true).message("Login Successful !!").userName(login.getUserName()).build();
                }else{
    ...
    public Response register(Register register) {
        User user = repo.findByUserName(register.getUserName());
        if(user==null) {
            if (register.getUserName().equals("") || register.getEmail().equals("") || register.getPassword().equals("")) {
            ...
            try {
                ...
                String userId = register.getUserName().substring(0,3)+register.getEmail().substring(0,2)+day+month+hrs+min;
            ...
            return Response.builder().userName(register.getUserName()).message("User Name Already Exist !!").status(false).build();
        }
        return Response.builder().userName(register.getUserName()).message("User Registered Successfully !!").status(true).build();
    }
}
